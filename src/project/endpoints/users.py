...
from project.schemas import usuario_schema, proveedor_schema
...
def register ():
    ...
    db.session.commit()   #confirmar session, agrega a bdd
...
def list(payload):
    ...
    return jsonify(usuario_schema.dump(usuarios, many=True)), 200  #transformar funciones#


@blueprint.route ('/users/<id>', methods=['GET'])  ##mismo nombre en view(ID)
@autenticar
def view(payload, id):
...
def update(payload, id):
    ...
    usuario = Usuario.query.get_or_404(id)
    usuario = usuario_schema.load(data=request.json, instance=usuario, partial = False)
 

    db.session.add(usuario)   # agregando un usuario a la sesion 
    db.session.commit()
...
def patch(payload,id):
     ...
     usuario = Usuario.query.get_or_404(id)
     usuario = usuario_schema.load(data=request.json, instance=usuario, partial = True)
...
def login():
    ...
    usuario = Usuario.query.filter_by(correo=correo).first()   #, password=password)
    ...
    return jwt.encode(payload, current_app.config['SECRET'], algorithm='HS256')  #clave del servidor. SECRETA.
